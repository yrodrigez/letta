...
function showSearch(){
    ...
    $.getScript('js/dao/events.js', function(){
        searchEvents(text,function (events) {
        	...
        	if(events.length<1){
	            $("#view-container").append("<h2 class='busqueda'>No se han encontrado coincidencias con: " + text +"</h2>");
        	}else{
	            $("#view-container").append("<h2 class='busqueda'>Resultados para: " + text +"</h2><br/>");
	        	var cont=0;
	            $.each(events, function (key, event) {
